...
@pytest.mark.asyncio
async def test_schedules(event_loop, login_fixture):
    async with login_fixture:
        async with aiohttp.ClientSession(loop=event_loop) as websession:
            ...
            assert schedules["s_1"] == schedules.get("s_1")

            all_sched = await schedules.all()
            assert len(all_sched) == 1
...
@pytest.mark.asyncio
async def test_schedules_error(event_loop, login_fixture):
    async with login_fixture:
        async with aiohttp.ClientSession(loop=event_loop) as websession:
            client = Client(websession)
            result = await client.login(email="test@test.com", password="password")
            ...
            with pytest.raises(KeyError):
                assert schedules["invalid"].enabled
